        ...
        <div v-if="product != null">
            ...
            <div v-if="product.reviews.length > 0">
                <div v-for="(review, index) in product.reviews" :key="index">
                    <strong>{{ review.reviewer }}</strong> (rating: {{ review.rating }}) - <a href="#" @click.prevent="deleteReview(review)">delete</a>
                    <p>{{ review.text }}</p>
                </div>
            </div>
...
export default {
  ...
  beforeRouteUpdate(to, from, next) {
    this.getProduct(to.params.productId).then(
    ...
    getProduct(productId) {
      return this.$http
        ...
        .then(
          ...
          response => alert("Could not retrive product")
        );
    },
    goBack() {
      ...
      this.reviewResource
        ...
        .then(
          response => {
            ...
            this.getProduct(this.product.id).then(
              product => (this.product = product)
            );
          },
